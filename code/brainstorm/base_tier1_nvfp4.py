...
def _fp8_bmm(X, Y):
    # X:(b,M,K) Y:(b,K,N) -> (b,M,N) via FP8 e4m3 with per row/col scales, fp32 accum
    b, M, K = X.shape
    N = Y.shape[-1]
    fp8 = torch.float8_e4m3fn
    sx = X.abs().amax(dim=-1, keepdim=True).clamp(min=1e-12) / 448.0  # 448 = e4m3 max
    sy = Y.abs().amax(dim=-2, keepdim=True).clamp(min=1e-12) / 448.0
